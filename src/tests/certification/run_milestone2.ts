...
async function main() {
  ...
  const runner = new CertificationRunner("2", "certification-2026");
  
  const manifestPath = path.join(__dirname, "milestone2.manifest.json");
  const testDir = path.join(__dirname, "milestone2");
  const reportDir = path.join(__dirname, "../../../reports/certification/milestone2");
