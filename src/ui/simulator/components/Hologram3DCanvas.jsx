...
export default function Hologram3DCanvas({ type, colorHex, isHovered = false }) {
  ...
  useEffect(() => {
    ...
    const pointLight = new THREE.PointLight(colorHex, 3.5, 10);
    pointLight.position.set(0, 2, 2);
    scene.add(pointLight);

    const baseLight = new THREE.PointLight(colorHex, 2, 5);
    baseLight.position.set(0, -1, 0);
    scene.add(baseLight);

    // 4. Multi-Ring 3D Holographic Pedestal
    const pedestalGroup = new THREE.Group();

    // Outer Torus Ring
    const torusGeo1 = new THREE.TorusGeometry(1.6, 0.025, 16, 64);
    const torusMat1 = new THREE.MeshBasicMaterial({ color: colorHex, wireframe: true, transparent: true, opacity: 0.7 });
    ...
    // Middle Ring
    const torusGeo2 = new THREE.TorusGeometry(1.25, 0.02, 16, 48);
    ...
    const cylinderGeo = new THREE.CylinderGeometry(0.9, 0.9, 0.04, 32);
    ...
    disc.position.y = -0.88;
    pedestalGroup.add(disc);

    scene.add(pedestalGroup);

    // 5. Floating 3D Main Object Group
    const modelGroup = new THREE.Group();

    // Construct specialized 3D geometry per app type
    if (type === 'IMAGE_STUDIO') {
      // 3D Poly Mountain / Crystal Peak
      const coneGeo = new THREE.ConeGeometry(1.1, 1.6, 7, 3);
      ...
      const innerConeGeo = new THREE.ConeGeometry(0.7, 1.2, 5, 2);
      ...
      innerCone.position.y = 0.1;
      modelGroup.add(innerCone);
    } else if (type === 'DOCUMENT_STUDIO') {
      // 3D Floating Clipboard & Stylus
      const boxGeo = new THREE.BoxGeometry(1.1, 1.5, 0.08);
      ...
      // 3D Stylus Pen
      const penGeo = new THREE.CylinderGeometry(0.04, 0.02, 1.2, 12);
      const penMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
      const pen = new THREE.Mesh(penGeo, penMat);
      pen.position.set(0.6, 0.2, 0.4);
      pen.rotation.z = -Math.PI / 4;
      pen.rotation.x = 0.2;
      modelGroup.add(pen);
    } else if (type === 'DATA_LAB') {
      // 3D Bar Columns + Floating Torus
      const bar1Geo = new THREE.BoxGeometry(0.22, 0.8, 0.22);
      const bar2Geo = new THREE.BoxGeometry(0.22, 1.3, 0.22);
      const bar3Geo = new THREE.BoxGeometry(0.22, 1.0, 0.22);
      const bar4Geo = new THREE.BoxGeometry(0.22, 1.5, 0.22);

      const mat = new THREE.MeshStandardMaterial({ color: colorHex, wireframe: true, emissive: colorHex, emissiveIntensity: 0.7 });
      
      const b1 = new THREE.Mesh(bar1Geo, mat); b1.position.set(-0.5, -0.4, 0); modelGroup.add(b1);
      const b2 = new THREE.Mesh(bar2Geo, mat); b2.position.set(-0.17, -0.15, 0); modelGroup.add(b2);
      const b3 = new THREE.Mesh(bar3Geo, mat); b3.position.set(0.17, -0.3, 0); modelGroup.add(b3);
      const b4 = new THREE.Mesh(bar4Geo, mat); b4.position.set(0.5, -0.05, 0); modelGroup.add(b4);

      // Floating 3D Pie Ring
      const pieGeo = new THREE.TorusGeometry(0.35, 0.08, 12, 24, Math.PI * 1.5);
      const pieMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
      const pie = new THREE.Mesh(pieGeo, pieMat);
      pie.position.set(0.4, 0.8, 0.2);
      pie.rotation.x = Math.PI / 3;
      modelGroup.add(pie);
    } else if (type === 'CODE_LAB') {
      // 3D Isometric Code Window with Tag
      const winGeo = new THREE.BoxGeometry(1.4, 1.0, 0.06);
      const winMat = new THREE.MeshStandardMaterial({ color: colorHex, wireframe: true, emissive: colorHex, emissiveIntensity: 0.6 });
      ...
      oct.position.set(0.5, -0.2, 0.4);
      modelGroup.add(oct);
    } else if (type === 'MEDIA_STUDIO') {
      // 3D Spinning Film Reel
      const reelGeo = new THREE.CylinderGeometry(0.7, 0.7, 0.1, 24);
      const reelMat = new THREE.MeshStandardMaterial({ color: colorHex, wireframe: true, emissive: colorHex, emissiveIntensity: 0.8 });
      const reel = new THREE.Mesh(reelGeo, reelMat);
      reel.rotation.x = Math.PI / 2.5;
      reel.position.set(-0.2, 0.2, 0);
      modelGroup.add(reel);

      // 3D Play Cone/Triangle
      const triGeo = new THREE.ConeGeometry(0.35, 0.5, 3);
      const triMat = new THREE.MeshBasicMaterial({ color: 0xffffff, wireframe: true });
      const tri = new THREE.Mesh(triGeo, triMat);
      tri.rotation.z = -Math.PI / 2;
      tri.position.set(0.5, -0.2, 0.3);
      modelGroup.add(tri);
    } else if (type === 'KNOWLEDGE_LAB') {
      // 3D Tesseract / Neural Cube
      const outerBoxGeo = new THREE.BoxGeometry(1.0, 1.0, 1.0);
      const outerBoxMat = new THREE.MeshStandardMaterial({ color: colorHex, wireframe: true, emissive: colorHex, emissiveIntensity: 0.8 });
      const outerBox = new THREE.Mesh(outerBoxGeo, outerBoxMat);
      modelGroup.add(outerBox);

      const innerBoxGeo = new THREE.BoxGeometry(0.55, 0.55, 0.55);
      ...
      const centerSphereGeo = new THREE.SphereGeometry(0.18, 12, 12);
  ...
  return (
    <div
      ref={mountRef}
      className="w-full h-36 flex items-center justify-center relative cursor-grab active:cursor-grabbing select-none"
    />
  );
}
