/**
 * ResponseGroundingGuard.mjs (v2.1 - Backend Non-Blocking, Traced & Positive-Evidence Grounding Guard)
 */

import { isCapabilityAvailable, isVerifiedSystemEntity } from './CapabilityRegistry.mjs';
...
export class ResponseGroundingGuard {
  ...
  guard(rawResponse, userPrompt = '', context = {}) {
    ...
    // 1. Minimum Sufficient Response Calibration
    ...
    if (isSimpleAudioRequest && text.length > 250 && !text.includes('```')) {
      ...
      console.log(`[TRACE] ${isoNow()} | GROUNDING_DONE | (Calibrated Simple Response)`);
      return {
    ...
    if (/^silakan\s+upload\s+audio|^silakan\s+unggah\s+audio/i.test(cleanUserPrompt)) {
      ...
      console.log(`[TRACE] ${isoNow()} | GROUNDING_DONE | (Normalized Upload Prompt)`);
      return {
    ...
    // 2. Positive-List System Entity Interceptor
    ...
    // 3. UI Reality & Freshness Guard
    const isAppActive = Boolean(context.activeApp && context.isSandboxRunning);
    if (!isAppActive) {
